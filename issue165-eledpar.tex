\documentclass[a4paper, oneside, 12pt]{book}
\makeatletter
\usepackage{eledmac,eledpar}
\numdef{\titi}{1}
 \sidenotemargin{left}
 \linenummargin{right}
 \setlength{\ledrsnotesep}{2\linenumsep}
\firstlinenum{1} \linenumincrement{1}

\begin{document}
\setcounter{tocdepth}{25}
\tableofcontents

\newpage
\begin{pages}
\begin{Leftside}
\beginnumbering

\pstart
\eledsection{\edtext{xxx}{\Afootnote{yyyy}}\ledsidenote{chapter}}
\pend
\pstart
DI use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten..
\pend
\pstart
\eledsection{xxxx\ledsidenote{section}}
\pend
\pstart
\eledsubsection{xxxx\ledsidenote{sub}}
\pend
\pstart
\eledsubsubsection{sssxxxx\ledsidenote{s}}
\pend
\pstart
\edtext{D}{\Afootnote{C}}I use s git to manage some package.
I would like to \ledsidenote{s}add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten.\edtext{E}{\Afootnote{F}}.
\pend
\pstart
\eledsection{\edtext{xsection}{\Afootnote{yyyy}}}
\pend
\pstart
\edtext{D}{\Afootnote{C}}I use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten.\edtext{E}{\Afootnote{F}}.
\pend


\endnumbering
\end{Leftside}

\begin{Rightside}
\beginnumbering

\pstart
\eledsection{\edtext{xxx}{\Afootnote{yyyy}}\ledsidenote{chapter}}
\pend
\pstart
DI use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten..
\pend
\pstart
\eledsection{xxxx section}
\pend
\pstart
\eledsubsection{xxxx subsection}
\pend
\pstart
\eledsubsubsection{sssxxxx subsubsection}
\pend
\pstart
\edtext{D}{\Afootnote{C}}I use s git to manage some package.
I would like to \ledsidenote{s}add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten.\edtext{E}{\Afootnote{F}}.
\pend
\pstart
\eledsection{\edtext{xsection}{\Afootnote{yyyy}}}
\pend
\pstart
\edtext{D}{\Afootnote{C}}I use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten.\edtext{E}{\Afootnote{F}}.
\pend


\endnumbering

\end{Rightside}
\Pages
\end{pages}


\newpage


\begin{pairs}
\begin{Leftside}
\beginnumbering

\pstart
\eledsection{\edtext{xxx}{\Afootnote{yyyy}}}
\pend
\pstart
DI use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten..
\pend
\pstart
\eledsection{xxx section}
\pend
\pstart
\eledsubsection{xxxx subsection}
\pend
\pstart
\eledsubsubsection{xxxx subsubsection}
\pend
\pstart
\edtext{D}{\Afootnote{C}}I use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten.\edtext{E}{\Afootnote{F}}.
\pend
\pstart
\eledsection{\edtext{xsection}{\Afootnote{yyyy}}}
\pend
\pstart
\edtext{D}{\Afootnote{C}}I use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten.\edtext{E}{\Afootnote{F}}.
\pend


\endnumbering
\end{Leftside}

\begin{Rightside}
\beginnumbering

\pstart
\eledsection{\edtext{xxx}{\Afootnote{yyyy}}}
\pend
\pstart
DI use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten..
\pend
\pstart
\eledsection{xxx section}
\pend
\pstart
\eledsubsection{xxxx subsection}
\pend
\pstart
\eledsubsubsection{xxxx subsubsection}
\pend
\pstart
\edtext{D}{\Afootnote{C}}I use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten.\edtext{E}{\Afootnote{F}}.
\pend
\pstart
\eledsection{\edtext{xsection}{\Afootnote{yyyy}}}
\pend
\pstart
\edtext{D}{\Afootnote{C}}I use s git to manage some package.
I would like to add a pre-commit hook, to check if I have forgotten
For now, I have a minimal pre-commit, which test only the blank spaces at ends of lines, but not the forgotten.\edtext{E}{\Afootnote{F}}.
\pend


\endnumbering

\end{Rightside}
\Columns
\end{pairs}

\end{document}
