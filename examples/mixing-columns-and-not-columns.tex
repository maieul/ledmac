...
\begin{english}
\title{Mixing critical text in two columns and in one column}
\maketitle
\begin{abstract}
This file provide an example of a a critical edition with part of text with parallel an other part without parallel. 

There is only one lineation for the left columns (when parallel exists) and for the main text (when no lineation). We use \verb+\pausenumbering+ and \verb+resumenumbering+ to do it.

To have text in one column aligned with text in two columns, we use \verb+\widthliketwocolumnstrue+, \verb+\Xnoteswidthliketwocolumns+ and \verb+\notesXwidthliketwocolumns+.
