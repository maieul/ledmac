...
\maketitle
\begin{abstract}
  This file show how to use the \verb+\doinsidethislinehook+ 
  command to use \verb+\markboth+ to add verse (sentence) number inside the headers.
  ...
  To understand this example, you must be familiar with \LaTeX\ marking mechanism (\verb+\markboth+) and \emph{fancyhdr} package.
\end{abstract}
\end{english}
...
\begin{abstract}
 ...
 \begin{itemize}
   \item typesets it with a bold fontface, following by a dot;
...
\begin{abstract}
 In this example, the verse (sentence) number is not typeset directly, but store in a \LaTeX\ counter (which name is \verb+verse+).
 ...
 Consequently, with hack, the \verb+\markboth+ macro won't have set the good value to the marker.

 However, we can solve this issue, because the argument of \verb+doinsidethislinehook+ is written in a auxiliary file
 at the time where  it is read.

Nevertheless, to write the correct value of the counter in the auxiliary file, we need to do some hacks with \TeX\ expansion's mechanism.
...
\newcommand{\autoversenumber}{%
  ...
  \expandafter\doinsidethislinehook\expandafter{\tmp}%First, we expand \tmp to  the value, ans then, pass the result as argument to \doinsidelinehook
  \textbf{\theverse}.\space%Typeset the verse counter value in bold, following by a dot and then a space.
}
