% !TeX encoding = UTF-8
% !TeX TS-program = xelatex
% !TeX spellcheck = it_IT
% !TeX root = Diogenes.tex
% !BIB TS-program = biber
% rubber: set program xelatex

\documentclass[11pt,a4paper,twoside]{book}
\usepackage{fontspec}
\setmainfont{Linux Libertine O}

\usepackage{polyglossia}
\setmainlanguage[babelshorthands=true]{italian}

%-----------------------------------------------------------------------
% INIZIO EDIZIONE CRITICA (LEDMAC)
%-----------------------------------------------------------------------
\usepackage{eledmac}

\lineation{page}    %% numerazione per pagina
\linenummargin{inner}   %% Margine dei numeri di linea
\sidenotemargin{outer}  %% Margine dei marginalia

\Xnotefontsize[A,B,C]{\footnotesize}

\renewcommand*{\ledlsnotefontsetup}%
    {\raggedleft\it\footnotesize}
\renewcommand*{\ledrsnotefontsetup}%
    {\raggedright\it\footnotesize}
\renewcommand*{\sidenotesep}%
    {~\emph{|}~}

\footparagraph{A}
\footparagraph{B}
\footparagraph{C}

%%%%% IF YOU UNCOMMENT ONE OF THESE, THE COMPILATION FAILS
%%%%% The first seems to be the standard; I would obtain a box of 2mm with a number inside it, right aligned, then a space of 3mm after the number
%\renewcommand{\thepstart}{{\bfseries\@arabic\c@pstart}. }
%\renewcommand{\thepstart}{{\makebox[2mm][r]{\bf\@arabic\c@pstart}}\hspace{3mm}}
%\bhookXnote{\renewcommand{\thepstart}{{\makebox[2mm][r]{\bf\arabic{pstart}}}\hspace{3mm}}}
%%%%%

%-----------------------------------------------------------------------
% FINE EDIZIONE CRITICA
%-----------------------------------------------------------------------

\begin{document}

\beginnumbering
\numberpstarttrue
\labelpstarttrue
\pstart\label{1}%
\ledsidenote{XX}%
\ledinnernote{bla}%
\ledouternote{bla2}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{1}}}\enspace Test}}
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart\label{2}%
\ledsidenote{XX}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{2}}}\enspace Test}}%
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart\label{3}%
\ledsidenote{XX}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{3}}}\enspace Test}}%
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart\label{4}%
\ledsidenote{XX}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{4}}}\enspace Test}}%
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart\label{5}%
\ledsidenote{XX}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{5}}}\enspace Test}}%
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart\label{6}%
\ledsidenote{XX}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{6}}}\enspace Test}}%
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart\label{7}%
\ledsidenote{XX}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{7}}}\enspace Test}}%
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart\label{8}%
\ledsidenote{XX}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{8}}}\enspace Test}}%
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart\label{9}%
\ledsidenote{XX}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{9}}}\enspace Test}}%
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart\label{10}%
\ledsidenote{XX}%
\edtext{}{\Afootnote[nonum,nosep]{{\textbf{\ref{10}}}\enspace Test}}%
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart
\ledsidenote{XX}
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart
\ledsidenote{XX}
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart
\ledsidenote{XX}
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\pstart
\ledsidenote{XX}
Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita Nel mezzo del cammin di nostra vita 
\pend

\numberpstartfalse
\endnumbering


\end{document}

%%% Local Variables:
%%% coding: utf-8
%%% mode: latex
%%% TeX-engine: xetex
%%% TeX-master: t
%%% End:
