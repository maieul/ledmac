...
\newcommand*{\longpara@vfootnote}[2]{%
  ...
  \ifdim\csuse{#1longpara@total}>\dimen\csname #1longfootins\endcsname%
    \global\setbox\csuse{#1keept@para}=\vbox{%
      \csuse{Xnotefontsize@#1}%
      \ifvoid\csuse{#1keept@para}\else%
        \csuse{#1keept@para}%
      \fi%
      \box0\penalty0%
      }%
...
\newcommand*{\longpara@footgroup}[1]{%
	  ...
	  \ifvoid0\else%
	    \ifdim\ht0<\baselineskip%
	      \vskip-\ht0\noindent\unvxh@l{0}
	    \else%
	     \global\setbox\csuse{#1@toolongpara}=\box0%
	    \fi%
	  \fi%
...
\def\continue@longnote{%
  \def\do##1{%
    \ifvoid\csuse{##1@toolongpara}\else%
      \setbox0=\box\csuse{##1@toolongpara}%
      \setbox2=\vbox{}%
      \vbadness=10000%
      \splittopskip=0pt%
      \csuse{Xnotefontsize@##1}%
