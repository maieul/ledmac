...
\pendtikz[A,B,C,D][\vskip2\baselineskip]
\pstarttikz[grow=right,sibling distance=4cm,
                      edge from parent/.style={draw,thick}]
   \node(A1){Prova}
        child {node[align=left] {Second line - \edtikztext{left}{\Afootnote{gauche}}{A1} } }
        child {node {Second line - right}
            child {node  { Third line - left}}
            child {node(B2) {Third line }
              child[level distance=2cm] {node{Fourth line}}
              }
        };
