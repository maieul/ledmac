...
\newcommand{\do@tikzline}[1]{%
  ...
  \ledllfill%
  \l@drd@ta%
  \hspace{2\paperwidth}X%To be correctly aligned
}
...
\newcommand{\pendtikz}{
  \foreach \Node in {A,tree_B,C,D} {%\y1=y-coord of the node
    \path let \p1=($ (\Node) $) in node at (0,\y1)[inner sep=0,outer sep=0,text width=\textwidth-\parindent]{\do@tikzline{\Node}};%
  }%
}
...
\pstart
  ...
  \noindent\begin{tikzpicture}[sibling distance=4cm,
   ...
   \node(A){Prova}
        child {node(tree_B)[align=left] {Second line - \edtikztext{left}{\Afootnote{gauche}}{tree_B} } }
